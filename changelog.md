...
- **修复失效测活被旧库存密码阻断（v2.25.3）**：
  ...
  - **测试 (Tests)**：`tests/test_any_auto_web_session_contract.py` 覆盖密码页 OTP 优先、发码 `2xx` 早于 SPA 跳转、密码拒绝后二次 OTP 兜底、库存密码不再重复提交及验证码状态继续推进；`tests/test_invalid_account_recheck.py` 锁定 `Incorrect email address or password` 的稳定分类。
