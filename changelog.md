...
### 修复 (Fixed)
- **修复注册面板邮箱画像保存失效**：`frontend/src/pages/Accounts.tsx` 现在会在用户显式点击“保存设置”时持久化本任务邮箱服务选择、TempMail 建箱模式和固定域名列表；注册弹窗重新打开时仅在存在这份显式画像时恢复对应值，否则继续以 `/api/config` 的共享默认配置为准。这样选择 `HME Ready API` 后不会再次被共享的 TempMail 域名画像覆盖，同时“开始注册”仍只携带任务级 `extra`，不会偷偷改写三实例共享配置。
