...
- **新增账号操作列对应的统一批量执行入口（v2.44.0）**：
  ...
  - **参数与危险操作 (Security)**：新增 `BatchAccountActionModal.tsx`，一次配置可安全复用于整批账号，API Key/Admin Key 使用密码输入框且留空时沿用系统设置。退出网页会话与彻底撤销 AT/RT 只允许处理明确勾选账号，并要求独立勾选批量确认；完整撤销使用危险按钮，Cookie-only 退出明确保持 AT/RT 不变。邮箱换绑仍必须逐账号冻结不同目标邮箱，不提供会破坏租约和身份校验边界的伪批量入口。
  - **补齐现有专用批量动作 (Changed)**：支付资格的“定向检测”菜单补充“批量检测 GCash 支付方式”，继续走既有 GCash 单项任务路由和国家/代理配置弹窗；支付链接、登录态、补抓 Auth、失效测活、状态同步、Sub2API/OAIPay 补传和批量删除继续复用原专用批量入口，不重复走通用同步执行器。
  - **列表与批量范围一致性 (Fixed)**：`api/accounts.py::list_accounts()` 仅在主列表查询不使用派生状态表时，为动态支付方式目录补刷一次过期索引；认证、手机号、支付链接、资格或远端状态等派生筛选已经在规范查询入口刷新时，不再于同一请求中二次重建状态。修复任务解析已冻结 1 个账号、列表随后却因第二次刷新返回 0 个账号的快照分裂，也减少筛选请求中的冗余 SQLite 写入。
  - **回归验证 (Tests)**：新增后端动作目录、危险动作元数据、共享参数逐账号透传测试，以及前端入口、选中优先/筛选回退、通用批量 API、确认与上限、GCash 菜单合同测试。隔离测试镜像完整收集 `1832 tests`，默认断网非 browser/live 回归 `1827 passed, 2 skipped, 3 deselected, 64 subtests passed`，筛选/目录/批量动作专项 `42 passed`；前端合同 `145/145 passed`，TypeScript/Vite 生产构建及新组件、工具栏、侧栏定向 ESLint 通过。侧栏版本同步为 `v2.44.0`。
- **新增支付方式国家-方式二级筛选与动态目录（v2.43.0）**：账号列表将原先只能按“有方式/无方式/未检测”判断的支付方式条件改为国家与支付方式绑定的结构化筛选。`services/chatgpt_core/payment_method_catalog.py` 统一 `card`、`link`、`gcash` 等原始 ID 与显示名称；`core/db.py` 新增账号、国家、方式三级唯一的 `account_payment_method_index`，只从最近一次确认结果建立索引，失败重试不会覆盖上次成功的国家和方式，明确无方式结果会清理旧索引，旧版 GCash 标记明确为不可用时也会清理对应索引。`api/accounts.py` 的 `GET /accounts` 与 `POST /accounts/query` 支持 `payment_method_selection`，并返回不受分页影响、但遵循当前其它筛选范围的 `payment_method_catalog`，因此每次列表刷新发现新的国家或支付方式都会自动进入级联目录。`frontend/src/pages/Accounts.tsx` 在桌面表头、移动端筛选、筛选预设和批量任务范围中共用国家 -> 支付方式多选控件，保持国家与方式的绑定，移除支付方式级联弹层的搜索框，已选国家和方式继续由受控筛选值保留；保留旧 `gcashPaymentMethodState` 和旧 GCash 标记的读取兼容，已选项在暂时无结果时仍可回显，新增项不会被自动选中。新增隔离回归覆盖 `VN -> link`、`PH -> gcash`、多组组合不交叉、完整目录不受分页影响、失败重试保留索引和无方式清理索引；侧栏版本同步为 `v2.43.0`。
...
- **将 auto-plus3 注册指纹从 macOS 精准回退到 Camoufox/Linux（v2.46.5）**：
  ...
  - **Linux 原生画像一致性 (Fixed)**：Plus3 新注册账号继续使用 Camoufox Firefox 147、每账号独立 device/profile/Canvas/Audio/字体/WebGL 和实际出口地理身份，但 UA、`navigator.platform/oscpu`、字体与 WebGL 预设全部改从 Camoufox Linux 数据生成；屏幕只从历史低失效率样本中最常见的 `1366x768..1920x1080` Linux 桌面档位选择，保留 `27px` 桌面面板、原生 `DPR=1` 和最大化 Firefox 内外窗口关系，排除手机尺寸与超宽统计预设。已有账号保存的 macOS 指纹不做数据库批量重写，只有新画像或原画像不兼容时才按当前运行时生成，避免无关账号登录态被强制旋转。
  - **可观测性与回归保护 (Changed/Tests)**：`GET /api/config` 增加不可写、不可共享的 `effective_deep_browser_operating_system`，任务元数据、控制日志、独立注册页、账号注册弹窗、Web Session 和设置页均按实例显示实际系统，Plus3 明确展示 `Firefox 147 on Linux`。隔离 Docker 完整收集 `1913 tests`，定向回归 `39 passed`，默认断网非 browser/live 全量回归 `1908 passed, 2 skipped, 3 deselected, 70 subtests passed`；真实 Xvfb/Camoufox Linux 双进程页面门禁 `1 passed`，完整前端合同 `162/162 passed`，TypeScript、Vite production build 与新增前端运行合同 helper 的定向 ESLint 通过。侧栏版本同步为 `v2.46.5`。
- **修复注册 OTP 假重发与 Checkout Sentinel 无令牌故障（v2.46.4）**：
  - **OTP 状态合同 (Fixed)**：`services/chatgpt_core/browser_registration.py` 与 `services/chatgpt_core/any_auto/browser_register.py` 将 HTTP 成功、OTP 业务提交和页面推进拆成独立状态。验证码提交后若 DOM 已进入 `about_you`、OAuth callback、ChatGPT 首页等下一状态，即使旧 validate URL 仍包含 `email-otp` 也按实时页面确认提交；只有 `HTTP 2xx + otp_committed=false + 页面确实仍为 OTP` 时才调用真实 `/email-otp/resend`，并且仅在 resend 返回 2xx 后增加验证码 generation、更新邮箱 cutoff 和等待第二代验证码。每个 challenge 最多真实重发一次，禁止在没有发信的情况下空等 120 秒或重复提交旧验证码。
  - **Sentinel 失败分层与重试 (Fixed)**：`services/chatgpt_core/browser_checkout.py` 和 `services/chatgpt_core/payment_eligibility.py` 将 SDK 空字符串、JSON 无法解析、缺少非空 `t`、Sentinel 准备超时分别归类为 `sentinel_token_missing`、`sentinel_token_incomplete`、`sentinel_attempt_timeout` / `sentinel_prepare_failed`，不再统一伪装成“checkout 创建 网络失败”。Sentinel 类失败在既有最大尝试预算内创建新的 BrowserContext、临时浏览器画像和动态代理 SID 后重试；最终 `reason_code` 保留具体失败码，任务日志只记录 token JSON 状态、字段存在性/长度、warmup 状态和耗时，不记录 token、telemetry、Cookie 或认证材料正文。
  - **出口与浏览器画像一致性 (Fixed)**：代理扫描确认的 Checkout 国家和出口 IP 通过仅存在于本次尝试的 `ResolvedProxyChain` 传入 Browser Checkout；`services/chatgpt_core/browser_identity.py::rebind_browser_fingerprint_geo()` 保留账号 device ID、浏览器预设、Canvas、Audio、字体及其它非地理熵，只把 locale、timezone、geolocation 和 WebRTC 重绑定到已验证出口，并让 BrowserContext、请求头及 Stripe init 共用该临时画像。临时地理画像不写回账号，避免一次国家化 Checkout 永久改变注册身份。
  - **有界请求与完成竞态 (Fixed)**：三个认证 warmup 改为并发执行，warmup、Sentinel token、ping、Checkout fetch 和响应体读取共用默认 `60s` 的页面内总 deadline；可通过 `AUTO_GPT_CHECKOUT_BROWSER_ATTEMPT_TIMEOUT_SECONDS` 在 `15..120s` 内调整，单个请求仍受 `30s` 上限及 AbortController 约束。`core/task_runtime.py` 同时将“达到注册目标、终止多余账号尝试”从人工/致命立即停止信号中拆出，正常完成不会再把已启动的 0 元资格检测改写为 `registration_task_stopped`，而目标达成后收到的真实人工停止仍可立即取消后处理。
  - **回归验证 (Tests)**：隔离 Docker 测试镜像完整收集 `1911 tests`；OTP、Checkout、Sentinel、出口画像、注册后资格写回和停止控制专项 `228 passed, 5 subtests passed`，默认断网非 browser/live 回归 `1906 passed, 2 skipped, 3 deselected, 70 subtests passed`，均为 `0 failed`。前端合同 `162/162 passed`，TypeScript 与 Vite production build 通过；侧栏版本同步为 `v2.46.4`。
...
- **修复批量确认弹窗被动作菜单遮挡（v2.44.2）**：
  ...
  - **回归验证 (Tests)**：前端合同新增“先关闭菜单、销毁子 Portal、后打开确认弹窗”的顺序约束，完整前端合同 `147/147 passed`，工具栏、新弹窗与侧栏定向 ESLint 及 TypeScript/Vite 生产构建通过。未发布构建在桌面打开完整撤销弹窗 `500ms` 后确认 Dropdown、二级 Portal 和可见菜单项均为 `0`，警告内容完整显示；手机仍为两个同层分组、页面 `scrollWidth=390`。版本同步为 `v2.44.2`。
...
- **统一迁移到 Plus3 的 Patchright Chromium 151 原生浏览器方案（v2.42.0）**：
  ...
  - **全链路统一与进程隔离 (Changed)**：注册、OAuth 捕获、Checkout、Web Session 登录、Sentinel/Auth 辅助浏览器和本地 Turnstile Solver 均切换到同一 Patchright Chromium；Solver 继续维护物理浏览器池并以多个独立 `BrowserContext` 隔离任务，注册、登录与 Checkout 继续保持“每个事务一个可强杀浏览器进程”，没有照搬 Plus3 的单进程共享池，从而保留现有停止、超时回收和故障隔离语义。辅助浏览器同步移除旧 Playwright Chromium 148、Windows UA、固定 viewport 和 `navigator.webdriver/window.chrome` 等注入。
  - **历史任务与回滚兼容 (Fixed)**：浏览器执行器不再接受客户端选择深浏览器内核，创建任务和 runner 执行前都会冻结为部署指定运行时，因此旧客户端传入 Firefox/Safari 以及升级前已排队任务也会使用当前 Patchright Chrome。历史 Camoufox/macOS 账号画像在下一次浏览器任务中迁移为 Patchright/Linux；Web Session 仅在登录成功后写回新画像，失败保留原画像。Camoufox `0.5.4` 与 Playwright 驱动只保留为显式 `CHATGPT_BROWSER_ENGINE=camoufox` 运维回滚路径，不发生静默跨内核降级。
  - **可观测性与界面 (Changed)**：运行快照新增 `browser_runtime=patchright`、`runtime_package_version=1.62.1`、`identity_surface=native_linux` 和 `mode=isolated_process_per_transaction`；任务元数据、日志、Checkout/Solver 文案和设置页明确展示 Patchright Chromium 151，默认浏览器族只控制纯协议执行器，侧栏版本同步为 `v2.42.0`。
  - **回归验证 (Tests)**：隔离测试镜像内确认 Patchright、curl-cffi 与 Chromium 版本，最终断网非 browser/live 回归 `1805 passed, 2 skipped, 2 deselected, 64 subtests passed`；OpenAI HTTP、OAuth、Checkout、Sentinel 与 Solver 定向回归 `313 passed, 15 subtests passed`；Xvfb 真实浏览器门禁覆盖 Chrome 151 UA、无 `HeadlessChrome`、Linux 平台、`navigator.webdriver=false`、原生 WebGL、无固定 viewport、跨 Context Cookie/localStorage 隔离及 Camoufox 显式回滚，共 `2 passed`；前端合同 `139/139 passed`，TypeScript/Vite 生产构建通过。

- **统一账号列表状态契约与字段语义（v2.41.0）**：`api/accounts.py` 为紧凑账号响应补充按“当前事实、最近尝试、历史确认”分层的列表摘要，`services/account_filters.py` 同步修正 AT 生命周期与认证有效性索引，避免 Codex 401、旧的账号活跃证据和 AT 到期时间互相覆盖。`frontend/src/pages/Accounts.tsx` 将认证材料、AT/RT 生命周期、业务流程、订阅计划、手机号/API、0 元资格、支付方式、支付链接、链接类型、集成上传和提交结果分别按单一职责呈现；历史值明确标注“上次确认”，失败原因降为次级信息。支付链接与链接类型继续作为两个独立且可分别开关的列，移动端与桌面端共用同一摘要规则。隔离 Docker 回归收集 1808 项，默认无网络回归 1804 passed、2 skipped、2 deselected，前端合同测试 139/139 通过。
...
- **将 auto-plus3 纳入统一多实例发布拓扑（v2.39.0）**：
  ...
  - **兼容 (Fixed)**：旧 `docker-compose.registration-node.yml` 明确标记为回滚/审计用途，避免它与新的 multi service 并行启动；统一发布文档、端口回环合同和拓扑测试同步覆盖第四实例，迁移门禁只接受已知的 `auto-plus3-local` 旧 Compose 归属，侧栏版本更新为 `v2.39.0`。
...
- **合并支付资格检测共享 Checkout 链路（v2.35.2）**：
  ...
  - **任务/API (Added)**：新增单账号与批量路由 `POST /api/tasks/chatgpt/payment-eligibility`、`POST /api/tasks/chatgpt/payment-eligibility/batch` 及公共国家/币种目录接口。组合任务沿用 0 元探测的国家代理一致性门禁，按支付方式预筛选，订阅账号继续允许支付方式检测，0 元与链接子结果保持原有“已订阅跳过”语义；原三个单项路由继续可用。
...
- **新增域名选择批量清空与运行任务批量停止（v2.35.0）**：
  ...
  - **运行任务面板 (Changed)**：`ActiveTasksPanel.tsx` 从只能打开单个任务的下拉框改为紧凑 Popover 批量控制面板，提供全选、逐项复选、刷新、已选数量、完成当前后停止和立即停止一次确认；普通任务保留独立日志入口，停止失败项继续保持勾选便于重试。`useActiveTasksQuery.ts` 仅在面板打开时每 `3` 秒刷新，终态任务会从列表和选择集合自动移除；活动摘要补充任务 `capabilities`，只有全部所选目标明确支持时才开放“完成当前后停止”。桌面面板限制高度并内部滚动，手机端使用单列停止按钮和视口约束宽度，长任务名称保持省略与完整 Tooltip/标题信息，不产生页面横向溢出。
  - **自动轮换边界 (Fixed)**：运行面板把同一 `registration_domain_task_group.mode=rotating` 的活动子任务聚合为一个逻辑目标，但仍逐个保留查看日志入口；整组停止沿用 `RegistrationDomainRotationManager.stop_group()`，同步取消等待域名、技术重试和后续补位。批量或旧版单任务 API 若试图只停止轮换子任务，会明确返回 `ROTATION_GROUP_REQUIRED`，禁止出现“停止当前子任务后调度器又启动下一个域名”的假停止；当前版 `TaskLogPanel.tsx` 的轮换子任务停止按钮则显式改为“停止整组”并调用现有任务组接口，不静默扩大旧请求范围。
  - **测试 (Tests)**：新增后端批量去重、幂等重复请求、终态竞态、不存在目标、不支持优雅停止、请求上限、活动摘要能力和轮换组保护测试；新增前端优选/本次选择清空、普通任务独立选择、轮换子任务归组、批量载荷、失败项保留、停止模式与实时刷新合同测试。前端合同 `115 passed`，TypeScript/Vite 生产构建和本次改动文件定向 ESLint 通过；隔离 Docker 完整收集 `1693 tests`，无外网、非 browser/live 回归 `1691 passed, 2 skipped, 56 subtests passed`。侧栏版本同步为 `v2.35.0`。

- **修复域名轮换技术异常误停整组（v2.34.1）**：
  - **根因 (Fixed)**：Plus3 的首个真实轮换组在启动约 `3` 秒后进入失败：`13146.asia` 尚未产生开户决策或注册账号，动态代理预检访问 `api.ipify.org` 时返回 `SSL: WRONG_VERSION_NUMBER`，旧状态机却把任意子任务 `failed` 都直接视为不可恢复的整组异常，导致其余 `14` 个同时活动域名全部收口。该错误属于实例代理/网络技术故障，不是域名拒绝率或提链质量，不能淘汰域名，也不应在单次瞬时失败时停止整组。
  - **异常恢复 (Changed)**：`services/chatgpt_core/registration_domain_rotation.py` 现在对动态代理、普通代理、TLS/SSL、网络、上游 `429/5xx`、超时、浏览器依赖和临时存储错误进行结构化分类。仅当失败任务没有开户决策、注册账号、成功/跳过计数或已完成进度时，才保留原域名槽位并使用冻结请求执行同域退避重试；默认最多重试 `2` 次，间隔为 `5s/15s`。一旦任务出现业务进度，禁止整任务重放，避免重复开户或超过每域名目标数；不可分类的程序错误仍立即停止补位并收口整组。
  - **基础设施熔断 (Changed)**：单个域名的技术重试耗尽后才进入整组故障；同一类技术错误在 `30s` 内命中 `2` 个不同域名时提前打开公共故障熔断，取消等待队列和尚未执行的重试，并让其它活动任务通过 `after_current` 完成在途账号后停止。某域名恢复到明确开户决策、注册账号或正常完成后，会立即清除其运行时故障票据，防止两个互不相关的瞬时错误被错误合并。服务重启仍不会恢复内存中的冻结凭据或重试定时器，旧活动组继续按原合同标记为 `interrupted`。
  - **可观察性 (Changed)**：持久组快照新增无凭据的 `failure`、最近技术故障、尝试次数、重试次数、下次重试时间和脱敏错误；每次重试继续创建普通可见注册任务，旧尝试不会从日志 Tabs 消失。`RegistrationDomainTaskGroupTabs.tsx` 区分“技术重试 / 技术失败 / 基础设施熔断”和质量淘汰，显示预计重试时间、`重试 n/2`、错误原因及历史尝试；配置区在活动槽位等于所选域名数时明确提示本次没有等待域名可补位，不再让“轮换”产生必然存在后备域名的错觉。
  - **测试 (Tests)**：新增 Plus3 实际 `动态代理没有可用候选 + SSL WRONG_VERSION_NUMBER` 签名、同域两次退避、重试耗尽、跨域同类错误熔断、恢复后故障票据清除、已有业务进度禁止重放、用户停止取消定时器、不可分类错误维持失败关闭、尝试历史序列化及前端归一化测试。隔离 Docker 相关回归 `135 passed, 2 subtests passed`，完整收集 `1688 tests`，断网非 browser/live 回归 `1686 passed, 2 skipped, 56 subtests passed`；前端完整合同 `109 passed`，TypeScript/Vite 生产构建和改动文件定向 ESLint 通过。侧栏版本同步为 `v2.34.1`。

- **新增优选域名质量熔断与自动轮换（v2.34.0）**：
  - **新增 (Added)**：`api/tasks.py` 与新增的 `services/chatgpt_core/registration_domain_rotation.py` 在原“合并任务 / 按域名拆分”之外增加“自动轮换”。运营可按优选域名顺序提交队列，并设置同时运行域名数（默认 `1`）；每个实际启动的域名仍由现有 `/api/tasks/register` 链路创建为普通、可见、可独立查看日志和停止的手动注册任务。新增 `GET /api/tasks/register/domain-groups/{group_id}` 与 `POST /api/tasks/register/domain-groups/{group_id}/stop`，用于读取动态补位状态和整组完成当前账号后停止；`core/db.py` 的 `registration_domain_rotation_groups` 只保存无凭据的调度审计快照。
  - **质量判定 (Changed)**：开户质量只统计明确的 `create_account` 接受与 `registration_disallowed` 拒绝；默认至少 `10` 个有效决策且拒绝率严格高于 `50%` 才触发。提链质量以注册账号的业务终态顺序计算，默认连续 `10` 个未提链成功触发；0 元不符合与账号级业务提链失败增加连续值，`link_succeeded/submitted/submit_failed` 均代表提链已成功并清零。代理、网络、超时、限流、上游 `5xx`、配置或写入故障、Auth 待补抓和等待态保持中性，既不增加也不清零，避免公共依赖异常误淘汰域名；不可逆 `create_account 2xx` 优先于后续重复请求拒绝文本，防止接受决策被反向误记。
  - **调度与恢复 (Changed)**：域名命中阈值后通过现有 `after_current` 控制完成所有在途账号并停止补尝试，只有子任务真正终态后才释放槽位并启动下一个等待域名；活动槽位大于 `1` 时始终按配置补齐且不绕过实例全局浏览器容量。任一子任务异常终态会停止补位、取消等待队列并让同组活动任务完成在途账号后收口，防止代理或公共服务故障扫空全部优选域名。服务重启会把旧活动组标为 `interrupted` 且绝不静默恢复；终态 TaskLog 即使暂时落库失败，轮换回调仍独立执行，避免槽位永久卡住。
  - **冻结与性能 (Changed)**：注册请求在组创建时一次性冻结有效邮箱、代理、浏览器、0 元检测、提链和支付配置，等待域名不会在数小时后重新读取已经变化的全局设置。冻结模板和逐账号提链 ledger 只存在于当前进程内，持久快照不写凭据、账号 ID 或逐账号明细；数据库只更新域名状态和聚合质量计数，组终态后立即释放模板及 ledger，避免大批量任务产生持续膨胀的 JSON 写放大。
  - **前端 (Changed)**：`RegistrationDomainTaskModeField.tsx`、`RegistrationDomainTaskGroupTabs.tsx`、`RegisterTaskPage.tsx` 与 `Accounts.tsx` 增加轮换模式、四个有界数字控制、每任务并发乘以活动槽位的请求上限提示、实时质量计数、等待/运行/淘汰/完成状态、淘汰原因和整组停止入口。活动组每 `2` 秒更新，浏览器标签隐藏时降频到 `10` 秒；新补位任务自动进入域名 Tabs 并切换日志，页面刷新会先按 `group_id` 恢复最新活动任务，不再只依赖旧任务 ID。手机端阈值输入改为单列，长域名与状态标签保持稳定宽度和省略，不产生横向页面滚动。
  - **测试 (Tests)**：新增轮换状态机、严格拒绝率边界、连续提链失败、成功清零、技术中性、并发槽位、整组停止、异常收口、无账号 ledger 持久化、重启中断、冻结请求和终态回调测试；PayPal 协调器回归锁定每账号结构化结果回调。隔离 Docker 完整收集 `1682 tests`，断网非 browser/live 回归 `1680 passed, 2 skipped, 56 subtests passed`；前端完整合同 `108 passed`，TypeScript/Vite 生产构建与本次新增文件定向 ESLint 通过。Playwright 在 `1280x900` 与 `390x844` 下确认模式、阈值、动态任务组和长域名均无重叠、越界或横向溢出；侧栏版本同步为 `v2.34.0`。
...
- **将高并发 Full 抓包收敛为单核心 Context（v2.33.6）**：
  ...
  - **测试 (Tests)**：`tests/test_registration_diagnostics.py` 新增容量 30 时视频门禁、核心制品完整性、正常路径单 Context、泛化 `Browser closed` 禁止再次创建诊断 Context，以及仅显式 screencast 能力错误可降级的回归。隔离 Docker 专项 `21 passed, 16 subtests passed`，注册/Worker/Sentinel 关联回归 `187 passed, 7 subtests passed`，完整收集 `1658 tests`，全量回归 `1656 passed, 2 skipped, 54 subtests passed`；前端合同 `101 passed`，TypeScript/Vite 生产构建通过。前端侧栏版本同步为 `v2.33.6`。
...
- **修复 OAIPay Plus 分类拉取仍走公网旧端口（v2.30.8）**：
  ...
  - **运维 (Changed)**：在备份并校验 `auto-gpt-plus/data/account_manager.db` 后，将 Plus 本地 `oaipay_api_url` 修复为内网地址；不改变 Plus 的共享配置脱离状态或其它实例的独立配置。
  - **测试 (Tests)**：新增 `:8789` 公网旧地址的归一化回归，验证内网分类接口返回分类列表，并在多实例发布后分别检查主服务、Plus、Plus2 的配置与分类连通性；前端侧栏版本同步为 `v2.30.8`。
...
- **延长 about_you 提交按钮等待并清理英国手机号池（v2.30.1）**：
  ...
  - **兼容 (Changed)**：前端侧栏版本同步为 `v2.30.1`；主实例 `auto-gpt` 与 `auto-plus2` 继续保持停止，仅 Plus 实例运行。
...
- **ChatGPT AT/RT 认证生命周期与账号证据可观测性（v2.27.0）**：
  ...
  - **前端交互优化 (Changed)**：账号列表将原先分散的“认证材料”“AT状态/到期”“RT刷新”“账号证据”合并为一个紧凑的“认证状态”列；列表保留材料类型、主认证结论和关键到期/刷新摘要，完整时间来源、证据码和探针历史继续放在账号详情，减少横向滚动和列数量。
...
### 修复 (Fixed)
- **补齐 Patchright 页面主世界调用，修复注册 Sentinel 与 Turnstile Solver（v2.42.2）**：全量审计 `services/chatgpt_core`、`services/turnstile_solver` 与 `api/chatgpt.py` 的 `evaluate/evaluate_handle` 调用后，确认除 `v2.42.1` 已修复的 Checkout 外还有两类同源遗漏。`services/chatgpt_core/sentinel_browser.py::_evaluate_complete_sentinel_token` 与 `sentinel_batch.py::PlaywrightSentinelProvider` 现在用 `isolated_context=False` 调用页面脚本安装的 `window.SentinelSDK.init/token/sessionObserverToken`，覆盖 OAuth 开户、注册 Sentinel 和手机号注册批量 token；`services/turnstile_solver/api_solver.py` 新增按浏览器运行时选择的页面世界入口，Patchright Chromium/Chrome/Edge 在主世界检测 `window.turnstile`、安装全局 token callback 并调用 Cloudflare `render`，Camoufox 回滚仍使用原生 Playwright evaluate 签名。其余调用经逐项核对仅访问共享 DOM、Navigator/Screen/WebGL 等浏览器内建对象或执行同源 `fetch`，旧 `platforms/chatgpt` Sentinel 实现仍使用 Playwright，因此没有机械扩大修改。定向无网络回归 `76 passed, 2 subtests passed`，Xvfb 真实 Chrome 151 与 Camoufox 浏览器门禁 `3 passed`；真实门禁同时证明 Patchright 默认隔离世界看不到页面 `SentinelSDK/turnstile`，三个修复入口均能读取并调用主世界对象。最终断网非 browser/live 回归 `1809 passed, 2 skipped, 3 deselected, 64 subtests passed`，前端合同 `139/139 passed` 且 TypeScript/Vite 生产构建通过，侧栏版本同步为 `v2.42.2`。

- **修复 Patchright 支付资格检测无法读取 Sentinel SDK（v2.42.1）**：`services/chatgpt_core/browser_checkout.py` 对 Patchright Checkout 页面统一使用 `page.evaluate(..., isolated_context=False)`，确保账号首页元数据、`window.SentinelSDK` 就绪检查、`token('chatgpt_checkout')`、telemetry、三段登录态 warmup 与 Checkout 创建请求都在页面主执行世界连续完成。`v2.42.0` 迁移后 Patchright 默认把 `evaluate()` 放入隔离世界，虽然主世界 SDK 已加载且 readiness 门禁通过，实际创建脚本仍看不到页面全局对象，导致 Plus3 任务 `task_1787934454271_e5bd0428` 及此前批次反复报 `Cannot read properties of undefined (reading 'token')`；本修复不修改账号、Access Token、Cookie、动态代理地区链或 Checkout 请求体。Camoufox 显式回滚继续调用原生 Playwright evaluate 签名，不接收 Patchright 专用参数；Checkout/支付资格定向回归 `76 passed`，Xvfb 真实 Chromium 与回滚门禁 `3 passed`，最终断网非 browser/live 回归 `1806 passed, 2 skipped, 3 deselected, 64 subtests passed`，前端合同 `139/139 passed` 且 TypeScript/Vite 生产构建通过，侧栏版本同步为 `v2.42.1`。
