...
def apply_browser_fingerprint(session, fingerprint: BrowserFingerprint):
    ...
    family = infer_browser_family(
        ...
        getattr(fingerprint, "impersonate", ""),
    )
...
def build_browser_headers(
    *,
    ...
    """构造与 Chrome / Firefox / Safari 画像一致的请求头。"""
    family = str(browser_family or "").strip().lower() or infer_browser_family(
        user_agent,
    )
    ...
    platform_version = str(sec_ch_platform_version or "").strip('"')
