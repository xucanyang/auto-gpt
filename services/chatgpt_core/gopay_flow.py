...
GOPAY_ERROR_TRANSLATIONS: tuple[tuple[str, str], ...] = (
    ...
    ("GoPay-119", "GoPay PIN 错误，请检查手机号对应 PIN"),
    ...
    ("midtrans charge: no reference", "Midtrans /charge 缺少 reference，请查看返回体诊断"),
    ("Midtrans /charge 缺少 reference", "Midtrans /charge 缺少 reference，请查看返回体诊断"),
)
...
def _snapshot(session: GoPaySession) -> dict[str, Any]:
    with session.lock:
        return {
            ...
            "active_action": session.active_action,
...
def _apply_gopay_checkout_locale(profile: dict[str, Any], country: str) -> dict[str, Any]:
    profile = dict(profile or {})
    normalized_country = normalize_checkout_country(country)
    locale, stripe_locale = GOPAY_CHECKOUT_LOCALES.get(
        normalized_country,
        (f"en-{normalized_country}", "en"),
    )
    profile["locale"] = locale
    profile["stripe_locale"] = stripe_locale
    return profile
...
def _select_gopay_browser_profile(session: GoPaySession, account: Any) -> dict[str, Any]:
    ...
    profile = _apply_gopay_checkout_locale(profile, session.country)
    with session.lock:
...
def _extract_midtrans_link_reference(data: dict[str, Any]) -> str:
    activation_link_url = str(data.get("activation_link_url") or "")
    m = re.search(r"reference=([a-f0-9-]{36})", activation_link_url)
    if not m:
...
class GoPayRunner:
    def __init__(self, session: GoPaySession, account: Any):
        ...
        proxy_seen = _proxy_observation(self.checkout_proxy)
        _safe_log(
            self.s,
            (
                "GoPay checkout proxy: "
    ...
    @staticmethod
    def _checkout_amount_from_init(init_resp: dict[str, Any]) -> tuple[Any, str]:
            ...
            if r.status_code in (406, 429):
                ...
                if r.status_code == 429:
                    ...
                    try:
                        ...
                        last_err = str((fallback_payload.get("error_messages") or ["?"])[0]) if isinstance(fallback_payload, dict) else str(fallback_payload)
                    except Exception:
                        last_err = fallback.text[:120]
                    _safe_log(self.s, f"Midtrans linking no-auth fallback status={fallback.status_code}")
    ...
    def _midtrans_create_charge(self) -> str:
        ...
        try:
            ...
            raise GoPayFlowError(f"Midtrans /charge 返回体不是 JSON: {r.text[:600]}") from exc
        link = str(data.get("gopay_verification_link_url") or "")
        _safe_log(self.s, f"Midtrans /charge gopay_verification_link_url: {link or '空'}")
        m = re.search(r"reference=([A-Za-z0-9]+)", link)
        if not m:
            _safe_log(self.s, f"Midtrans /charge 缺少 reference，解析对象: {_safe_json_summary(data, 1200)}")
            raise GoPayFlowError(f"Midtrans /charge 缺少 reference: gopay_verification_link_url={link!r}")
...
def create_gopay_session(
    ...
    *,
    ...
    session = GoPaySession(
        ...
        pin_source=str(pin_source or ("全局默认PIN" if default_pin else "未配置")).strip(),
        billing=billing,
    ...
    _safe_log(session, f"GoPay PIN 来源: {session.pin_source or '未配置'}，状态: {'已配置' if session.default_pin else '未配置'}")
...
def probe_chatgpt_checkout_amount(
    ...
    *,
    ...
    amount, amount_source = runner._checkout_amount_from_init(init_resp)
...
def submit_gopay_otp(session_id: str, account: Any, otp: str) -> dict[str, Any]:
    ...
    if not session:
        raise KeyError(session_id)
    normalized_otp = re.sub(r"\D", "", str(otp or ""))
    ignore_duplicate = False
    with session.lock:
        if session.phase != PHASE_WAITING_OTP:
            if session.phase in {PHASE_WAITING_LINK_PIN, PHASE_WAITING_PAYMENT_PIN, PHASE_VERIFYING, PHASE_SUCCEEDED}:
                ignore_duplicate = True
            else:
                raise GoPayFlowError(f"当前阶段不需要 OTP: {session.phase}")
        elif session.active_action:
            ignore_duplicate = True
        else:
            session.active_action = "otp"
            session.last_otp = normalized_otp
            session.updated_at = _utcnow_iso()
    if ignore_duplicate:
        _safe_log(session, f"重复 OTP 提交已忽略: phase={session.phase} action={session.active_action}")
        return _snapshot(session)
    try:
        _get_runner(session).submit_otp_until_link_pin(normalized_otp)
    except Exception as exc:
        error = _translate_gopay_error_message(exc)
        ...
        with session.lock:
            if session.active_action == "otp":
                session.active_action = ""
                session.updated_at = _utcnow_iso()
    return _snapshot(session)
...
def submit_gopay_pin(session_id: str, account: Any, pin: str) -> dict[str, Any]:
    ...
    ignore_duplicate = False
    with session.lock:
        if session.phase not in {PHASE_WAITING_LINK_PIN, PHASE_WAITING_PAYMENT_PIN}:
            if session.phase in {PHASE_VERIFYING, PHASE_SUCCEEDED}:
                ignore_duplicate = True
            else:
            ...
            session.active_action = "pin"
            session.updated_at = _utcnow_iso()
    if ignore_duplicate:
        _safe_log(session, f"重复 PIN 提交已忽略: phase={session.phase} action={session.active_action}")
        return _snapshot(session)
    try:
