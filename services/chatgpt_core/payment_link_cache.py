...
IDEAL_EXPIRED_CLEANED_STATUS = "ideal_expired_cleaned"
IDEAL_PAID_CLEANED_STATUS = "ideal_paid_cleaned"
IDEAL_CANCELLED_CLEANED_STATUS = "ideal_cancelled_cleaned"
IDEAL_CLEANED_STATUSES = frozenset({
    IDEAL_EXPIRED_CLEANED_STATUS,
    IDEAL_PAID_CLEANED_STATUS,
    IDEAL_CANCELLED_CLEANED_STATUS,
})
PAYMENT_LINK_DELETED_STATUS = "payment_link_deleted"
PAYMENT_LINK_CLEANED_STATUSES = frozenset({
    *PIX_CLEANED_STATUSES,
    *UPI_CLEANED_STATUSES,
    *IDEAL_CLEANED_STATUSES,
...
PAYMENT_LINK_STATUS_LABELS = {
    ...
    IDEAL_EXPIRED_CLEANED_STATUS: "iDEAL 已过期清理",
    IDEAL_PAID_CLEANED_STATUS: "iDEAL 已支付清理",
    IDEAL_CANCELLED_CLEANED_STATUS: "iDEAL 支付已取消清理",
    PAYMENT_LINK_DELETED_STATUS: "支付链接已删除",
}
...
# ChatGPT Checkout catalog mirrored from openai-pay-long-link's 2026-07-25
# COUNTRY_CURRENCY snapshot.  These are Checkout-configured currencies, not
# inferred ISO national currencies.  The upstream profile API currently returns
# only the selected pair, so Auto-GPT exposes this server-side mirror to its UI.
