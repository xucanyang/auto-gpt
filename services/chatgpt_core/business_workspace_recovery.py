...
class BusinessWorkspaceRecovery:
    ...
    def list_available_teams(self) -> List[Dict[str, Any]]:
        ...
        if result and result.get("success"):
            ...
            if isinstance(teams, list):
                self._log(f"business recovery: 可用 team 数量={len(teams)}", "debug")
                for team in teams:
                    try:
                        self._log(
                            f"business recovery: team候选 id={int((team or {}).get('id') or 0)} members={int((team or {}).get('current_members') or 0)}/{int((team or {}).get('max_members') or 0)} plan={str((team or {}).get('subscription_plan') or '-')} expires_at={str((team or {}).get('expires_at') or '-')}",
                            "debug",
                        )
                    except Exception:
                        pass
                return teams
        error_text = str((result or {}).get("error") or (result or {}).get("message") or "unknown").strip()
        self._log(f"business recovery: 获取可用 team 失败: {error_text or 'unknown'}", "debug")
        return []
    ...
    def invite_member(self, email: str, team_id: int) -> tuple[bool, bool, str]:
        ...
        self._log(
            f"business recovery: invite_member result team={team_id} ok={ok} exhausted={exhausted} error={error_text or '-'}",
            "debug",
        )
        if ok:
    ...
    def send_invite_only_for_account(
        ...
        *,
        ...
        if preferred_team_ids:
            self._log(
                f"business recovery: 预选 team_id={','.join(str(tid) for tid in preferred_team_ids)}",
                "debug",
            )
