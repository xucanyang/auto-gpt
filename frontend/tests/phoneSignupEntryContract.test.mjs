...
test('the ChatGPT account modal submits the existing phone signup contract', () => {
  ...
  assert.match(handler, /const concurrency = normalizeRegisterConcurrency\([\s\S]+?phoneSignupEnabled \|\| resolvedMailProvider === 'manual_email_otp'/)
  assert.match(handler, /count: values\.count,\s*concurrency,/)
