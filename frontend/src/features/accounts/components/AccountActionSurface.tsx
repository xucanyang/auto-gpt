...
export function AccountActionSurface({
  ...
  const runAction = async (action: any, params: Record<string, any> = {}) => {
    ...
    if (actionId === 'zero_amount_eligibility' && onPaymentEligibilityTask) {
      await onPaymentEligibilityTask(acc, 'zero_amount_eligibility')
      return
    }
    if (actionId === 'payment_methods' && onPaymentEligibilityTask) {
