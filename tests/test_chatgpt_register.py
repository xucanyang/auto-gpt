...
class RefreshTokenRegistrationEngineTests(unittest.TestCase):
    ...
    @mock.patch("services.chatgpt_core.refresh_token_registration_engine.OAuthClient")
    @mock.patch("services.chatgpt_core.refresh_token_registration_engine.ChatGPTClient")
    def test_run_captures_plus_gopay_link_after_refresh_token_registration(
        self,
        mock_chatgpt_client_cls,
        _mock_oauth_client_cls,
        _mock_oauth_manager_cls,
        ...
        engine = self._make_engine(
            extra_config={
                ...
                "chatgpt_checkout_country": "ID",
                "chatgpt_checkout_currency": "IDR",
                "chatgpt_gopay_defaults": '{"billing_name":"Michael Anderson"}',
            }
        )
        full_auth = {
            "account_id": "acct-personal",
        ...
        provider_snapshot = {
            ...
            "cs_id": "cs_live_123",
            "payment_platform_url": "https://app.midtrans.com/snap/v4/redirection/demo",
            "midtrans_redirect_url": "https://app.midtrans.com/snap/v4/redirection/demo",
        ...
        with mock.patch.object(
            ...
            "core.proxy_utils.resolve_default_chatgpt_proxy",
        ...
        self.assertTrue(result.success)
        self.assertEqual(result.refresh_token, "rt-full")
        self.assertEqual(result.metadata["chatgpt_checkout_plan"], "plus")
        self.assertTrue(result.metadata["chatgpt_gopay_provider_link_ready"])
        self.assertEqual(
            result.metadata["chatgpt_gopay_provider_link"],
            provider_snapshot["payment_platform_url"],
        )
        generate_link.assert_called_once()
        create_provider.assert_called_once()
        self.assertEqual(create_provider.call_args.kwargs["plan"], "plus")
        self.assertEqual(create_provider.call_args.kwargs["country"], "ID")

    @mock.patch("services.chatgpt_core.refresh_token_registration_engine.OAuthManager")
    @mock.patch("services.chatgpt_core.refresh_token_registration_engine.OAuthClient")
    @mock.patch("services.chatgpt_core.refresh_token_registration_engine.ChatGPTClient")
    def test_gopay_failure_does_not_fail_refresh_token_registration(
        self,
        mock_chatgpt_client_cls,
        _mock_oauth_client_cls,
        _mock_oauth_manager_cls,
        ...
        mock_chatgpt_client_cls.return_value = self._registered_client()
        engine = self._make_engine(
            extra_config={"chatgpt_gopay_provider_link_enabled": True}
        )
        full_auth = {
            "account_id": "acct-personal",
            "workspace_id": "ws-personal",
        ...
        with mock.patch.object(
            ...
            "services.chatgpt_core.payment.generate_plus_link",
            side_effect=RuntimeError("checkout unavailable"),
        ):
            result = engine.run()
        ...
        self.assertIn(
            ...
            result.metadata["chatgpt_gopay_provider_link_error"],
        )
