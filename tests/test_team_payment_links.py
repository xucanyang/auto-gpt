...
class TeamPaymentLinkContractTests(unittest.TestCase):
    ...
    def test_team_partial_overrides_are_allowed_but_invalid_or_ambiguous_inputs_fail(self) -> None:
        ...
        self.assertEqual(filtered["billing_country"], "US")
        ...
        self.assertEqual(custom["billing_country"], "CH")
    ...
    def test_variant_key_changes_for_workspace_coupon_interval_and_profile(self) -> None:
        ...
        self.assertEqual(len({base_key, *(payment_link_variant_key(item) for item in variants)}), 8)

        request_params = normalize_payment_link_params({
            "plan": "team",
            "billing_country": "fr",
            "checkout_proxy_region": "CA",
        })
    ...
    def test_team_action_preserves_legacy_paypal_and_persists_redacted_history(self) -> None:
        ...
        with Session(engine) as session:
            ...
            self.assertEqual(history.get_result()["billing_country"], "GB")
    ...
    def test_plugin_sends_team_overrides_to_profile_and_batch(self) -> None:
        ...
        self.assertEqual(result["data"]["billing_country"], "CH")
        effective_overrides = {
            key: value
            for key, value in overrides.items()
            if key not in {"country", "currency"}
        }
        effective_overrides["checkout_ui_mode"] = "hosted"
