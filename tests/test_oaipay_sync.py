...
class OaiPaySyncTests(unittest.TestCase):
    ...
    def test_backfill_keeps_workspace_gate_for_plus_access_token_only_account(self):
        account = self._make_account()
        extra = account.get_extra()
        extra.pop("refresh_token")
        ...
        with mock.patch(
            "services.oaipay_sync.upload_to_oaipay_detailed",
            side_effect=AssertionError("upload must not run without workspace"),
        ) as upload:
            result = backfill_chatgpt_account_to_oaipay(account, commit=False)

        self.assertFalse(result["ok"])
        self.assertTrue(result["skipped"])
        self.assertEqual(result["capabilities"]["upload_gate"], "blocked_missing_workspace")
        upload.assert_not_called()
    ...
    def test_direct_upload_rejects_free_access_token_only_account_without_network(self):
        account = self._make_account()
        extra = account.get_extra()
        extra.pop("refresh_token")
        extra["chatgpt_local"] = {"subscription": {"plan": "free"}}
        account.set_extra(extra)
        ...
        self.assertFalse(result["ok"])
        self.assertTrue(result["skipped"])
        self.assertEqual(result["upload_gate"], "blocked_missing_rt")
        get.assert_not_called()
        post.assert_not_called()
