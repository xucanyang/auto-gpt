...
class AnyAutoWebSessionContractTests(unittest.TestCase):
    ...
    def test_otp_error_summary_keeps_rate_limit_without_marking_deactivated(self):
        ...
        self.assertIn("code=invalid_code", summary)
        self.assertIn("Too many tries", summary)
        self.assertNotIn("account_deactivated", summary)

    @unittest.skipUnless(_CAMOUFOX_AVAILABLE, "camoufox is only installed in the runtime image")
    def test_otp_error_summary_distinguishes_cloudflare_html_from_deactivation(self):
        response = mock.Mock()
        response.status = 403
        response.json.side_effect = ValueError("not json")
        response.text.return_value = "<html><title>Just a moment...</title></html>"

        summary = browser_register._auth_response_error_text(response)

        self.assertIn("HTTP 403", summary)
        self.assertIn("Just a moment", summary)
        self.assertNotIn("account_deactivated", summary)

    @unittest.skipUnless(_CAMOUFOX_AVAILABLE, "camoufox is only installed in the runtime image")
    def test_otp_error_summary_keeps_bodyless_403_unresolved(self):
        response = mock.Mock()
        response.status = 403
        response.json.side_effect = ValueError("not json")
        ...
        self.assertEqual(summary, "HTTP 403")
